...
export function MultiSelectFilter({
  ...
  return <div className="space-y-2">
        ...
        <PopoverContent className="w-80 p-0" align="start">
          <div className="p-3 border-b">
            <div className="flex items-center justify-between">
              ...
              {selectedValues.length > 0 && <Button variant="ghost" size="sm" onClick={handleClearAll} className="h-6 px-2 text-xs">
                  Limpar todos
                </Button>}
            </div>
          </div>
      ...
      {selectedValues.length > 0 && <div className="flex flex-wrap gap-1 mt-2">
          {selectedValues.map(value => {
        const option = options.find(opt => opt.value === value);
        return <Badge key={value} variant="secondary" className="text-xs">
                {option?.label || value}
                <button onClick={() => handleValueToggle(value)} className="ml-1 hover:bg-destructive/20 rounded-full p-0.5">
                  <X className="h-3 w-3" />
                </button>
              </Badge>;
      })}
